...
- 新增 `MacSvnGlobalExperiencePresentation.swift`：`MacSvnGlobalKeyboardPage` / `MacSvnGlobalKeyboardContract`、`MacSvnMotionPolicy`、`MacSvnCommandRShortcutModifier`；
- 新增 `HumanCenteredGlobalExperienceTests.swift`（10 项）：契约表、MotionPolicy、各页 ⌘F/⌘R 与 `macSvn.<page>.*` 源码门禁、嵌入 Diff/Commit 禁用 ⌘R、Commit 引用 MotionPolicy、U7 辅助页不回归；
- Changes：`@FocusState` + 隐藏 Button ⌘F、刷新 ⌘R、`macSvn.changes.search/refresh`；
- Log / Branches / Conflicts / Repo / Properties / Locks / Shelve / Settings：补齐或保持 ⌘R 与 identifier；Log/辅助页保持 ⌘F；
- Diff / Commit：独立页 ⌘R；嵌入工作区 `MacSvnCommandRShortcutModifier(enabled: !embedded)`；Commit 折叠动画走 `MacSvnMotionPolicy.run`；
- AuxiliaryPathList 可选 `searchAccessibilityIdentifier`。

### 任务 5 门禁与窗口

| 门禁 | 结果 |
|------|------|
| U8 定向 HumanCenteredGlobalExperienceTests | 10/10 |
| HumanCentered* + Modal + Perf + Settings + L10n 相关 | 与全量一并绿 |
| 全量 `swift test` | **1138/1138** |
| 真实 SVN `SvnCliBackendIntegrationTests` | **49/49** |
| `build-macos-app.sh` → `dist/SVNStudio.app` | 通过 |
| `verify-macos-app.sh` / `smoke-test-macos-app.sh` | 通过 |
| `git diff --check` | 通过 |

真实窗口截图（不入库）：`artifacts/ui/u8-changes-dark-980x640.png`、`u8-log-light-1180x760.png`、`u8-settings-light-reduce-motion-1440x900.png`、`u8-branches-dark-980x640.png`、`u8-properties-light-1180x760.png`、`u8-locks-light-reduce-motion-1440x900.png`、`u8-shelve-dark-980x640.png`、`u8-workspace-light-1180x760.png`、`u8-repo-dark-1440x900.png`（Retina 2×：980→1960×1384、1180→2360×1520、1440→2880×1800）。

### residual 与边界

- VoiceOver / 真实按键：自动化契约完成；`AXIsProcessTrusted=false` / System Events 记 residual，不砍功能；
- inventory / H-tortoise 未改（无能力状态变化）；
- 不重启 Tortoise Perfect Loop，不创建 wake/heartbeat；
- **Human UI 长程目标（U5–U8）完成**。

最终提交：`feat(UI): 完成人本全局体验收口（U8）`。
