...
| 2026-07-14 | T5.5 | （提交后回填） | 外置 Diff/Merge/Blame 规则按用途和扩展名持久化；大小写无关精确规则优先，留空/`*`/`*.*` 为默认；Diff 保留旧配置兜底，Merge 使用 base/mine/theirs/result 且不自动 resolve，Blame 限制工作副本边界；全量 937 绿 | S10、H-T5 外置工具 ✅ | 覆盖率 108/114（94.74%）；下一 T5.6 |
