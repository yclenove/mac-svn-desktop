...
|----|------|
| 日期 | 2026-07-21 |
| 实现提交 | 任务 1–4：`882e1a1` / `8f6dad6` / `b1a4318` / `65436be`；任务 5 本切片 |
| ST 定向 | `HumanCenteredSpecializedToolsTests` **12/12** |
| 全量 | `swift test` **1150/1150** |
| 真实 SVN | `SvnCliBackendIntegrationTests` **49/49** |
| App 门禁 | `build-macos-app` / `verify-macos-app` / `smoke-test-macos-app` / `git diff --check` 通过 |
| 英文 L10n | 补齐 ST 刷新/错误文案 5 键：`刷新 Blame`、`刷新 AI 助手上下文`、`刷新 Release Notes 日志`、`Blame 加载失败`、`已刷新路径列表` |
| 截图（不入库） | 见下表 `artifacts/ui/st-*.png`（Retina 2×） |

### 10.1 真实窗口截图

| 文件 | 页面 · 外观 · 逻辑尺寸 |
|------|------------------------|
| `st-blame-dark-980x640.png` | Blame · 深色 · 980×640（像素 1960×1384） |
| `st-blame-light-1180x760.png` | Blame · 浅色 · 1180×760（像素 2360×1520） |
| `st-aiAssistant-light-1180x760.png` | AI 助手 · 浅色 · 1180×760 |
| `st-aiAssistant-light-reduce-motion-1440x900.png` | AI 助手 · 浅色 + Reduce Motion · 1440×900（像素 2880×1800） |
| `st-gitMigration-dark-980x640.png` | Git 迁移 · 深色 · 980×640 |
| `st-gitMigration-light-reduce-motion-1440x900.png` | Git 迁移 · 浅色 + Reduce Motion · 1440×900 |
| `st-releaseNotes-dark-980x640.png` | Release Notes · 深色 · 980×640 |
| `st-releaseNotes-light-1180x760.png` | Release Notes · 浅色 · 1180×760 |

### 10.2 现场偏差与 residual

- **独立页 HSplitView**：Blame / AI 助手保留单层 `HSplitView`（H≤1、V=0 源码门禁）；嵌入工作区路径仍禁止 SplitView。
- **VoiceOver / 真实按键**：与 U6–U8 同口径——自动化契约 + a11y identifier；宿主 `AXIsProcessTrusted=false` 时动态 VO 遍历与真实按键注入仍为 residual。
- **UI testing 隔离 HOME**：无登记工作副本时 Blame / Release Notes 等为空态/引导态，属预期；工具栏、反馈条与刷新入口仍可达。
- **Git Migration 无 ⌘R**：契约 `requiresRefreshShortcut(for: .gitMigration) == false`，以执行/对账标识符与 busy 门禁覆盖。
- **能力未降级**：AI 写工具确认门 + 审计、迁移对账失败阻断同步（NFR-14）、Blame 外置/范围/演化解释均保留。
- **inventory / H-tortoise**：无能力状态变化，未修改。
- **Developer ID / 公证**：凭据阻塞仍在包装层 residual（非 ST 范围）。

### 10.3 U 系列与后续边界

- Human UI **U5–U8 已完成**；本波次为 **ST**，不重启 Tortoise Perfect Loop / wake / heartbeat。
- ST 完成后不自动开启新波次；后续体验迭代需用户显式新开规划。
