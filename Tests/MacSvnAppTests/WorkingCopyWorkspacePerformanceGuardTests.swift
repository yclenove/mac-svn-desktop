...
final class WorkingCopyWorkspacePerformanceGuardTests: XCTestCase {
    ...
    func testConflictWorkspaceAvoidsSplitViews() throws {
        let source = try Self.readFeatureSource(named: "MacSvnConflictWorkspaceView.swift")

        XCTAssertFalse(
            source.contains("HSplitView {"),
            "冲突主从区禁止 HSplitView，应使用稳定 HStack"
        )
        XCTAssertFalse(
            source.contains("VSplitView {"),
            "冲突主从区禁止 VSplitView，应使用稳定 VStack"
        )
    }

    func testPropertiesWorkspaceAvoidsSplitViews() throws {
        let source = try Self.readFeatureSource(named: "MacSvnPropertiesView.swift")

        XCTAssertFalse(
            source.contains("HSplitView {"),
            "属性目标与详情禁止自由 HSplitView，应使用稳定 HStack"
        )
        XCTAssertFalse(
            source.contains("VSplitView {"),
            "属性信息、列表与编辑器禁止自由 VSplitView，应使用稳定 VStack"
        )
    }
